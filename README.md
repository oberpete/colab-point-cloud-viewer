...
- **Peer list overlay with last-seen** — the top-right panel lists every connected peer with their color, ID, and a coarse last-seen indicator (`just now`, then 10s chunks, then whole minutes). The camera heartbeat pauses when a tab is backgrounded (Page Visibility API), so an idle peer's entry visibly greys out and its timestamp starts climbing *before* the WebSocket actually disconnects — distinguishing "connected but inactive" from "actively viewing." The timestamp is the server's, sent on `init` and `peer_update` (`shared/types.ts`), so a freshly loaded page shows each peer's real last-active time instead of resetting everyone to "just now."
